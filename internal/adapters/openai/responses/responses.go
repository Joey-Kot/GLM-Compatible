	...
	for i := 0; i < len(items); i++ {
		...
		case itemType == "message":
			if shared.StringValue(item["role"]) == "assistant" {
				...
				for ; j < len(items); j++ {
					candidate := items[j]
					if !IsToolCallItem(candidate) || (item["_upstream_turn_id"] != nil && !SameUpstreamTurn(item, candidate)) {
						break
					}
		...
		case IsToolCallItem(item):
			...
			for ; j < len(items); j++ {
				candidate := items[j]
				if !IsToolCallItem(candidate) || (item["_upstream_turn_id"] != nil && candidate["_upstream_turn_id"] != nil && !SameUpstreamTurn(item, candidate)) {
					break
				}
				calls = append(calls, candidate)
			}
...
func ResponseMessageToChatMessage(item shared.Map) shared.Map {
	...
	message := shared.Map{"role": role, "content": item["content"]}
...
func SameUpstreamTurn(left, right shared.Map) bool {
...
func UpstreamToolNameForResponseItem(item shared.Map) string {
	...
	return shared.SafeUpstreamToolName(shared.RawUpstreamToolName(item["name"], shared.StringValue(item["namespace"])))
}

func (a Adapter) BuildUpstreamPayload(payload shared.Map, messages []shared.Map) (shared.Map, map[string]shared.Map) {
	model := shared.StringValue(payload["model"])
	if model == "" {
		model = a.DefaultModel
	}
	messages = chat.MessagesForGLMModel(messages, model)
	...
	tools, toolNameMap := NormalizeToolsForUpstream(payload["tools"])
...
func NormalizeToolsForUpstream(value any) ([]shared.Map, map[string]shared.Map) {
...
func NormalizeToolDefinition(raw any, used map[string]string, toolNameMap map[string]shared.Map, namespace, namespaceDescription string) shared.Map {
	...
	case "web_search", "web_search_preview", "web_search_preview_2025_03_11", "web_search_2025_08_26":
		...
		return chat.GLMWebSearchTool(tool)
	default:
		return nil
	}
}
...
func NormalizeFunctionTool(tool map[string]any, used map[string]string, toolNameMap map[string]shared.Map, namespace, namespaceDescription string) shared.Map {
	...
	upstreamName := shared.UniqueUpstreamToolName(originalName, namespace, used)
...
func AllowedEntryMatches(entry any, mapping shared.Map) bool {
	if text, ok := entry.(string); ok {
		return text == mapping["name"] || text == mapping["upstream_name"] || text == shared.RawUpstreamToolName(mapping["name"], shared.StringValue(mapping["namespace"]))
	}
...
func MapTextFormat(payload shared.Map, messages []shared.Map, chatPayload shared.Map) {
	...
	case "json_schema":
		chatPayload["response_format"] = shared.Map{"type": "json_object"}
		if instruction := chat.JSONSchemaInstruction(formatConfig); instruction != "" {
...
func ResponseUsageFromUpstream(value any) any {
...
func mappedToolNameForChoice(name any, namespace, toolType string, toolNameMap map[string]shared.Map) string {
	...
	return shared.SafeUpstreamToolName(shared.RawUpstreamToolName(want, namespace))
}
