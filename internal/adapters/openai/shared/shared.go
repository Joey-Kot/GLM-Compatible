...
func RawUpstreamToolName(name any, namespace string) string {
...
func SafeUpstreamToolName(raw string) string {
	safe := strings.Trim(toolNameUnsafe.ReplaceAllString(raw, "_"), "_")
	if safe == "" {
		safe = "tool"
	}
	if len(safe) <= MaxUpstreamToolNameLength {
		return safe
	}
	digest := shortHash(raw, 12)
	prefixLen := MaxUpstreamToolNameLength - len(digest) - 1
	return strings.TrimRight(safe[:prefixLen], "_-") + "_" + digest
}

func UniqueUpstreamToolName(name any, namespace string, used map[string]string) string {
	raw := RawUpstreamToolName(name, namespace)
	candidate := SafeUpstreamToolName(raw)
	...
	for suffix := 2; ; suffix++ {
		...
		s := fmt.Sprint(suffix)
		prefixLen = MaxUpstreamToolNameLength - len(digest) - len(s) - 2
		base = candidate
		if len(base) > prefixLen {
			base = base[:prefixLen]
		}
...
func EstimateTokensFromMessages(messages []Map) int {
...
func estimateTokensFromMessages(messages []Map) int {
	...
	for _, message := range messages {
		parts = append(parts, StringValue(message["content"]))
	}
	...
	extra := (asciiChars - asciiWords*4) / 4
	if extra < 0 {
		extra = 0
	}
	total := asciiWords + nonASCII + extra
	if total < 1 {
		return 1
	}
	return total
