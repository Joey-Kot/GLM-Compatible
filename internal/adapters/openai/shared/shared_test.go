...
func TestSafeAndUniqueUpstreamToolName(t *testing.T) {
	if got := SafeUpstreamToolName("mcp tool.with spaces"); got != "mcp_tool_with_spaces" {
		t.Fatalf("SafeUpstreamToolName = %q", got)
	}
	long := strings.Repeat("a", 100)
	if got := SafeUpstreamToolName(long); len(got) > MaxUpstreamToolNameLength {
		t.Fatalf("long tool name was not capped: %q", got)
	}
	used := map[string]string{}
	first := UniqueUpstreamToolName("tool/a", "", used)
	second := UniqueUpstreamToolName("tool a", "", used)
	if first == second {
		t.Fatalf("colliding raw names produced same upstream name: %q", first)
	}
}
...
func TestEstimateTokensFromMessages(t *testing.T) {
	...
	if got := EstimateTokensFromMessages([]Map{{"content": "hello world"}, {"content": "你好"}}); got <= 0 {
