...
func (a Adapter) BuildUpstreamPayload(model string, payload shared.Map) (Prepared, error) {
	if model == "" {
...
func ContentToChatMessages(content map[string]any) []shared.Map {
	...
	for _, raw := range parts {
		...
		if part["text"] != nil {
			appendText(shared.StringValue(part["text"]))
			continue
		}
		...
		if glmPart := GeminiMediaPart(part); glmPart != nil && role == "user" {
			contentParts = append(contentParts, glmPart)
			continue
		}
		appendText(DescribeUnsupportedPart(part))
	}
	flushContent()
	if len(messages) == 0 {
...
func GeminiMediaPart(part map[string]any) shared.Map {
	if inline, ok := part["inlineData"].(map[string]any); ok {
		url := GeminiDataURL(inline)
		return GLMPartForMIME(shared.StringValue(inline["mimeType"]), url)
	}
	if inline, ok := part["inline_data"].(map[string]any); ok {
		url := GeminiDataURL(inline)
		return GLMPartForMIME(shared.StringValue(inline["mime_type"]), url)
	}
	if file, ok := part["fileData"].(map[string]any); ok {
		return GLMPartForMIME(shared.StringValue(file["mimeType"]), shared.StringValue(file["fileUri"]))
	}
	if file, ok := part["file_data"].(map[string]any); ok {
		return GLMPartForMIME(shared.StringValue(file["mime_type"]), shared.StringValue(file["file_uri"]))
	}
	return nil
}
...
func GLMPartForMIME(mimeType string, url string) shared.Map {
	if url == "" {
		return nil
	}
	switch {
...
func DescribeUnsupportedPart(part map[string]any) string {
	return "\n[Gemini part omitted by GLM compatibility layer: " + shared.JSONString(part) + "]\n"
}
...
func MapGenerationConfig(config map[string]any, messages []shared.Map, chatPayload shared.Map) {
	...
	if schema := config["responseSchema"]; schema != nil {
		chatPayload["response_format"] = shared.Map{"type": "json_object"}
		instruction := chat.JSONSchemaInstruction(map[string]any{"type": "json_schema", "name": "response", "schema": schema})
...
func NormalizeToolChoice(value any, allowed map[string]bool) any {
	...
	for _, rawName := range namesList(fc["allowedFunctionNames"]) {
		allowed[shared.SafeUpstreamToolName(rawName)] = true
	}
...
func ResponseFromUpstream(completion shared.Map, model string) shared.Map {
	if model == "" {
	...
	return shared.Map{
		...
		"usageMetadata": UsageFromUpstream(completion["usage"]),
		"modelVersion":  model,
...
func StreamChunkFromUpstream(chunk shared.Map, model string) shared.Map {
	...
	return ResponseFromUpstream(converted, model)
}

func CountTokens(payload shared.Map) (shared.Map, error) {
	messages, err := NormalizeContents(payload)
	if err != nil {
		return nil, err
	}
	count := shared.EstimateTokensFromMessages(messages)
	return shared.Map{"totalTokens": count}, nil
}
