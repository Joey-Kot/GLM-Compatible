...
func (c *Client) Chat(ctx context.Context, payload shared.Map) (shared.Map, error) {
	return c.postJSON(ctx, c.chatURL(), payload, "chat")
}
...
func (c *Client) StreamChat(ctx context.Context, payload shared.Map, handle func(shared.Map) error) error {
	req, err := c.newRequest(ctx, c.chatURL(), payload)
	if err != nil {
		return err
	}
	req.Header.Set("Accept", "text/event-stream")
	if c.DebugLogBody {
		log.Printf("debug body glm stream request url=%s body=%s", req.URL.String(), debuglog.MarshalBody(payload))
	}

	resp, err := c.httpClient().Do(req)
	if err != nil {
		return fmt.Errorf("external GLM stream failed: %w", err)
	}
	...
	if resp.StatusCode >= 400 {
		...
		return HTTPError{StatusCode: resp.StatusCode, Message: glmErrorMessage(data, string(body))}
	}
	...
	for scanner.Scan() {
		...
		if c.DebugLogBody {
			log.Printf("debug body glm stream response data=%s", debuglog.Body([]byte(text)))
		}
		if text == "[DONE]" {
			break
		}
...
func (c *Client) postJSON(ctx context.Context, url string, payload shared.Map, label string) (shared.Map, error) {
	...
	if c.DebugLogBody {
		log.Printf("debug body glm %s request url=%s body=%s", label, req.URL.String(), debuglog.MarshalBody(payload))
	}
	...
	if c.DebugLogBody {
		log.Printf("debug body glm %s response status=%d body=%s", label, resp.StatusCode, debuglog.Body(body))
	}
	var data shared.Map
	_ = json.Unmarshal(body, &data)
	if resp.StatusCode >= 400 {
		return nil, HTTPError{StatusCode: resp.StatusCode, Message: glmErrorMessage(data, string(body))}
	}
	if data == nil {
		return nil, HTTPError{StatusCode: http.StatusBadGateway, Message: "GLM returned a non-JSON response"}
	}
	return data, nil
}

func (c *Client) newRequest(ctx context.Context, url string, payload shared.Map) (*http.Request, error) {
	if c.APIKey == "" {
		return nil, HTTPError{StatusCode: http.StatusInternalServerError, Message: "GLM API key is not configured"}
	}
...
func (c *Client) chatURL() string {
	return c.endpointURL("/paas/v4/chat/completions")
}

func (c *Client) tokenizerURL() string {
	return c.endpointURL("/paas/v4/tokenizer")
}
...
func glmErrorMessage(data shared.Map, fallback string) string {
	if data != nil {
		if errObj, ok := data["error"].(map[string]any); ok {
			if message := shared.StringValue(errObj["message"]); message != "" {
				return "External GLM error: " + message
			}
		}
		...
		if code := shared.StringValue(data["code"]); code != "" {
			return "External GLM error: " + code
		}
	}
	return "External GLM error: " + fallback
}
