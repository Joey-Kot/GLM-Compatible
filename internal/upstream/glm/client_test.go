...
func TestChatPostsJSONAndParsesResponse(t *testing.T) {
	...
	client.HTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		if r.URL.String() != "https://glm.test/api/paas/v4/chat/completions" {
	...
	response, err := client.Chat(context.Background(), shared.Map{"model": "glm-5.1"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestChatUsesExplicitChatCompletionsURL(t *testing.T) {
	client := New("https://glm.test/custom/paas/v4/chat/completions", "sk-upstream", time.Second, true)
	client.HTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		if r.URL.String() != "https://glm.test/custom/paas/v4/chat/completions" {
			t.Fatalf("url = %s", r.URL.String())
		}
...
func TestTokenizePostsToTokenizerEndpoint(t *testing.T) {
	client := New("https://glm.test/api", "sk-upstream", time.Second, true)
	client.HTTPClient = &http.Client{Transport: roundTripFunc(func(r *http.Request) (*http.Response, error) {
		if r.URL.String() != "https://glm.test/api/paas/v4/tokenizer" {
			t.Fatalf("url = %s", r.URL.String())
		}
		return jsonResponse(http.StatusOK, `{"usage":{"total_tokens":12}}`), nil
	})}
	response, err := client.Tokenize(context.Background(), shared.Map{"model": "glm-5.1"})
	if err != nil {
		t.Fatal(err)
	}
	usage, _ := response["usage"].(map[string]any)
	if shared.IntValue(usage["total_tokens"], 0) != 12 {
		t.Fatalf("response = %#v", response)
	}
}
...
func TestDebugLogBodyLogsRedactedUpstreamBodies(t *testing.T) {
	...
	_, err := client.Chat(context.Background(), shared.Map{"model": "glm-5.1", "api_key": "secret-value"})
	if err != nil {
		t.Fatal(err)
	}
	text := logs.String()
	if !strings.Contains(text, "debug body glm chat request") || !strings.Contains(text, "debug body glm chat response") {
...
func TestNewConfiguresTLSVerification(t *testing.T) {
	verified := New("https://glm.test/api", "sk-upstream", time.Second, true)
	if isInsecureSkipVerify(verified.HTTPClient) {
		t.Fatal("expected TLS verification to be enabled by default")
	}

	unverified := New("https://glm.test/api", "sk-upstream", time.Second, false)
...
func TestNewWithTransportConfig(t *testing.T) {
	client := NewWithTransportConfig("https://glm.test/api", "sk-upstream", time.Second, true, TransportConfig{
		MaxIdleConns:        20,
