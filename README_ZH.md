...
| `--store-ttl` | 本地内存 Store 的 TTL，单位为秒，默认 `3600`；设为 `0` 关闭 TTL。 |
| `--store-max-responses` | 本地最多保存的 Responses 条目数，默认 `1000`；设为 `0` 关闭该上限。 |
| `--store-max-chat-completions` | 本地最多保存的 Chat Completions 条目数，默认 `1000`；设为 `0` 关闭该上限。 |
| `--store-max-conversations` | 本地最多保存的 Conversations 条目数，默认 `1000`；设为 `0` 关闭该上限。 |
| `--store-prune-interval` | 请求路径上两次 Store 清理检查的最小间隔，单位为秒，默认 `60`。 |
| `--glm-http-timeout` | GLM 上游 HTTP 请求超时时间，单位为秒，默认 `120`。 |
| `--glm-max-response-body-bytes` | GLM 上游非流式响应体或错误响应体大小上限，单位为字节，默认 `33554432`；设为 `0` 关闭该限制。 |
...
| `--max-request-body-bytes` | 本地请求体大小上限，单位为字节，默认 `16777216`；设为 `0` 关闭该限制。 |
...
| `--debug-pprof` | 是否启用需鉴权访问的 `/debug/pprof/` 和 `/debug/vars` 端点，默认 `false`。 |
...
| `GET /health` | 健康检查端点。 |
| `GET /healthz/memory` | 需鉴权访问的内存与本地 Store 统计端点。 |
...
本后端会在内存中保存 Responses、Chat Completions 和 Conversations 状态，用于支持 `previous_response_id`、`conversation`、读取、删除、列表过滤、metadata 更新和输入项列表等兼容能力。如果没有接入外部存储，服务重启后这些本地状态会丢失。

### 内存 Store 回收

本地 `store=true` 的状态默认会在请求路径上主动清理，不依赖后台 goroutine：
...
超过容量上限时按保存顺序淘汰对应类型的旧条目；淘汰 Response 或 Conversation 时会同步清理不再被任何 Response 或 Conversation 引用的 `ItemsByID` 条目。请求体默认限制为 `--max-request-body-bytes 16777216`，上游非流式响应体和错误响应体默认限制为 `--glm-max-response-body-bytes 33554432`；设为 `0` 可关闭对应限制。
...
如需 pprof，可显式启用 `--debug-pprof=true`，然后访问已鉴权的 `/debug/pprof/` 或 `/debug/vars`。默认不会暴露调试端点。
